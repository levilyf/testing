...
const testRipgrepOnFirstUse = memoize(async (): Promise<void> => {
  ...
  try {
    ...
    // For embedded ripgrep, use Bun.spawn with argv0 (Bun-only: single-file
    // `bun build --compile` binaries dispatch on argv[0]). Not reachable on
    // Node since isInBundledMode() only returns true under that Bun mode,
    // but guarded here so it degrades to the builtin/vendor path instead of
    // throwing a ReferenceError if that assumption ever changes.
